...
public sealed class EmojiReactionsServiceTests : ReactionsServiceTestsBase
{
    ...
    public void GetGuildEmojiReactionsTests()
    {
        ...
        TestDbProvider.SetupAlterAndVerify(
            ...
            _ => {
                ...
                Assert.That(
                    ers.Single(er => er.Response == Emojis.Cake.GetDiscordName() && er.Triggers.Single() == "abc"),
                    Is.Not.Null);
                Assert.That(
                    ers.Single(er => er.Response == Emojis.ArrowUp.GetDiscordName() && er.Triggers.Single() == "abc"),
                    Is.Not.Null);
                Assert.That(
                    ers.Single(er => er.Response == Emojis.ArrowDown.GetDiscordName() && er.Triggers.Single() == "abc"),
                    Is.Not.Null);
            }
        );
    ...
    public async Task AddEmojiReactionTests()
    {
        await TestDbProvider.SetupAlterAndVerifyAsync(
            ...
            async db => {
                ...
                this.Service.LoadData();
                Assert.That(
                    await this.Service.AddEmojiReactionAsync(MockData.Ids[0], Emojis.Information.GetDiscordName(),
                        new[] {"test"}, false),
                    Is.EqualTo(1)
                );
            },
            db => {
                ...
                Assert.That(
                    x.Single(er =>
                        er.Response == Emojis.Information.GetDiscordName() && er.DbTriggers.Single().Trigger == "test"),
                    Is.Not.Null
                );
                return Task.CompletedTask;
            }
        ...
        await TestDbProvider.AlterAndVerifyAsync(
            async db => {
                ...
                Assert.That(
                    await this.Service.AddEmojiReactionAsync(MockData.Ids[1], Emojis.Information.GetDiscordName(),
                        new[] {"testing"}, false),
                    Is.EqualTo(1)
                );
            },
        ...
        await TestDbProvider.AlterAndVerifyAsync(
            async db => {
                ...
                Assert.That(
                    await this.Service.AddEmojiReactionAsync(MockData.Ids[0], Emojis.AlarmClock.GetDiscordName(),
                        new[] {"regex(es)? (much)+"}, false),
                    Is.EqualTo(1)
                );
                Assert.That(
                    await this.Service.AddEmojiReactionAsync(MockData.Ids[0], Emojis.Information.GetDiscordName(),
            ...
            db => {
                ...
                Assert.That(ers.Any(e =>
                    e.IsMatch("here regex(es)? (much)+ will match because this is literal string interpretation")));
        ...
        await TestDbProvider.AlterAndVerifyAsync(
            ...
            db => {
                ...
                Assert.That(info.IsMatch("This is another TEST rEGex example which passes"));
                Assert.That(info.IsMatch("This is a protesting regexes example which should not pass due to wb check"),
                    Is.False);
        ...
        await TestDbProvider.SetupAlterAndVerifyAsync(
            ...
            db => {
                ...
                Assert.That(info.IsMatch("This is another tEst regex example which passes"));
                Assert.That(
                    info.IsMatch("This is a another protesting regexes example which should not pass due to wb check"),
                    Is.False);
                Assert.That(info.IsMatch("This is a tEst which wont pass"), Is.False);
                Assert.That(info.IsMatch("This is a literal test(ing)? regex(es)? string which wont pass"), Is.False);
    ...
    public async Task RemoveEmojiReactionByEmojiTests()
    {
        await TestDbProvider.SetupAlterAndVerifyAsync(
            ...
            async db => {
                ...
                this.Service.LoadData();
                Assert.That(
                    await this.Service.RemoveEmojiReactionsAsync(MockData.Ids[0], Emojis.Information.GetDiscordName()),
                    Is.Zero);
            },
            db => {
                Assert.That(db.EmojiReactions, Has.Exactly(this.erCount.Sum(kvp => kvp.Value)).Items);
        ...
        await TestDbProvider.SetupAlterAndVerifyAsync(
            ...
            db => {
                Assert.That(db.EmojiReactions, Has.Exactly(this.erCount.Sum(kvp => kvp.Value) - 1).Items);
                Assert.That(this.Service.GetGuildEmojiReactions(MockData.Ids[0]),
                    Has.Exactly(this.erCount[0] - 1).Items);
                Assert.That(this.Service.GetGuildEmojiReactions(MockData.Ids[1]), Has.Exactly(this.erCount[1]).Items);
                Assert.That(
                    this.Service.GetGuildEmojiReactions(MockData.Ids[0])
                        .Any(er => er.Response == Emojis.Chicken.GetDiscordName()), Is.False);
                Assert.That(
                    db.EmojiReactions
                        .AsQueryable()
                        .Where(er => er.GuildIdDb == (long)MockData.Ids[0])
                ...
                Assert.That(
                    this.Service.GetGuildEmojiReactions(MockData.Ids[2])
                        .Single(er => er.Response == Emojis.Chicken.GetDiscordName()), Is.Not.Null);
                Assert.That(
                    db.EmojiReactions
                        .AsQueryable()
    ...
    public async Task RemoveEmojiReactionTriggersTests()
    {
        ...
        await TestDbProvider.SetupAlterAndVerifyAsync(
            ...
            db => {
                ...
                Assert.That(this.Service.GetGuildEmojiReactions(MockData.Ids[0]), Has.Exactly(this.erCount[0]).Items);
                Assert.That(this.Service.GetGuildEmojiReactions(MockData.Ids[1]),
                    Has.Exactly(this.erCount[1] - 3).Items);
                return Task.CompletedTask;
            }
        );
    }
    ...
    public async Task RemoveAllEmojiReactionsTests() =>
        await TestDbProvider.SetupAlterAndVerifyAsync(
            ...
            db => {
                ...
                Assert.That(this.Service.GetGuildEmojiReactions(MockData.Ids[0]), Has.Exactly(this.erCount[0]).Items);
                Assert.That(this.Service.GetGuildEmojiReactions(MockData.Ids[1]), Is.Empty);
                Assert.That(db.EmojiReactions.AsQueryable().Select(tr => tr.GuildId),
                    Does.Not.Contain(MockData.Ids[1]));
                return Task.CompletedTask;
            }
        );
    ...
    private void AddMockReactions(TheGodfatherDbContext db)
    {
        db.EmojiReactions.Add(new EmojiReaction {
            ...
            DbTriggers = new HashSet<EmojiReactionTrigger> {new() {Trigger = "not"}}
        });
        db.EmojiReactions.Add(new EmojiReaction {
            GuildId = MockData.Ids[0],
            Response = Emojis.Headphones.GetDiscordName(),
            DbTriggers = new HashSet<EmojiReactionTrigger> {new() {Trigger = "ab+c"}}
        });
        db.EmojiReactions.Add(new EmojiReaction {
            GuildId = MockData.Ids[0],
            Response = Emojis.Chicken.GetDiscordName(),
            DbTriggers = new HashSet<EmojiReactionTrigger> {new() {Trigger = "abc"}}
        });
        db.EmojiReactions.Add(new EmojiReaction {
            GuildId = MockData.Ids[0],
        ...
        db.EmojiReactions.Add(new EmojiReaction {
            ...
            DbTriggers = new HashSet<EmojiReactionTrigger> {new() {Trigger = "abc"}, new() {Trigger = "cde"}}
        });

...
| ``channel``<br>``channels``<br>``c``<br>``chn`` | ``delete``<br>``-``<br>``d``<br>``del``<br>``remove`` | Manage Channels | ``(optional) [channel] Channel/Category``<br><br>``(optional) [text] Reason`` | Delete channel or category. If channel is not given as argument, deletes the current channel. | ``!channels delete``<br><br>``!channels delete #afkchannel`` |

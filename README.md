...
| ``channel``<br>``channels``<br>``c``<br>``chn`` | ``createtext``<br>``createtxt``<br>``createt``<br>``ctxt``<br>``ct``<br>``+``<br>``+t``<br>``+txt`` | Manage Channels | ``[string] Name``<br><br>``(optional) [channel] Parent Category (def: none)`` | Create new text channel. *Note: Discord does not allow spaces in text channel name.* | ``!channels +t spam``<br><br>``!channels +t spam public`` |
| ``channel``<br>``channels``<br>``c``<br>``chn`` | ``createvoice``<br>``createv``<br>``cvoice``<br>``cv``<br>``+voice``<br>``+v`` | Manage Channels | ``[string] Name``<br><br>``(optional) [channel] Parent Category (def: none)`` | Create new voice channel. | ``!channel createvoice "My Voice Channel"``<br><br>``!channels +v "My Voice Channel" "My category"`` |

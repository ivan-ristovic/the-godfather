...
| ``guild``<br>``server``<br>``g`` | ``setleavechannel``<br>``setleavec``<br>``setwc``<br>``setleave`` | Manage guild (user) |  | Set current leave message channel for this guild. If not specified, the current channel is set. | ``!guild setlc``<br>``!guild setlc #general`` |

...
|---|---|---|---|---|---|
|   |   |   |   |   |   |
| ``channel``<br>``channels``<br>``c``<br>``chn`` | ``createcategory``<br>``createcat``<br>``createc``<br>``+category``<br>``cat``<br>``cc``<br>``+cat``<br>``+c`` | Manage Channels | ``[text] Name`` | Create new channel category. | ``!channels +c My Category`` |
| ``channel``<br>``channels``<br>``c``<br>``chn`` | ``createtext``<br>``createtxt``<br>``createt``<br>``ctxt``<br>``ct``<br>``+``<br>``+t``<br>``+txt`` | Manage Channels | ``[string] Name``<br><br>``(optional) [channel] Parent Category (def: none)``<br><br>``(optional) [bool] NSFW? (def: false)`` | Create new text channel. *Note: Discord does not allow spaces in text channel name.* | ``!channels +t spam``<br><br>``!channels +t spam public true`` |
| ``channel``<br>``channels``<br>``c``<br>``chn`` | ``createvoice``<br>``createv``<br>``cvoice``<br>``cv``<br>``+voice``<br>``+v`` | Manage Channels | ``[string] Name``<br><br>``(optional) [channel] Parent Category (def: none)``<br><br>``(optional) [int] User limit (def: unlimited)``<br><br>``(optional) [int] Bitrate (def: 64000)`` | Create new voice channel. | ``!channel createvoice "My Voice Channel"``<br><br>``!channels +v "My Voice Channel" "My category" 10 96000`` |
| ``channel``<br>``channels``<br>``c``<br>``chn`` | ``delete``<br>``-``<br>``d``<br>``del``<br>``remove``<br>``rm`` | Manage Channels | ``(optional) [channel] Channel/Category (def: current)``<br><br>``(optional) [text] Reason`` | Delete channel or category. If channel is not given as argument, deletes the current channel. | ``!channels delete``<br><br>``!channels delete #afkchannel`` |
...
| ``channel``<br>``channels``<br>``c``<br>``chn`` | ``modify``<br>``edit``<br>``mod``<br>``m``<br>``e`` | Manage Channels | ``[channel] Channel/Category``<br><br>``(optional) [int] User limit``<br><br>``(optional) [int] Bitrate``<br><br>``(optional) [text] Reason`` | Modify a given voice channel. Set 0 if you wish to keep the values as they are. | ``!channel modify 5 64000 "reason"``<br><br>``!channel mod 0 32000`` |
| ``channel``<br>``channels``<br>``c``<br>``chn`` | ``rename``<br>``r``<br>``name``<br>``setname`` | Manage Channels | ``[string] Name``<br><br>``(optional) [channel] Channel/Category (def: current)``<br><br>``(optional) [text] Reason`` | Rename a channel. If a channel is not given as argument, renames the current channel. | ``!channel rename New Name``<br><br>``!channel rename "New Name" "Channel" "some reason for renaming"`` |
| ``channel``<br>``channels``<br>``c``<br>``chn`` | ``setparent``<br>``setpar``<br>``parent``<br>``par`` | Manage Channels | ``[channel] Parent category``<br><br>``(optional) [channel] Child (def: current)``<br><br>``(optional) [text] Reason`` | Change the parent of the given channel. | ``!channel setparent Category #somechannel`` |
| ``channel``<br>``channels``<br>``c``<br>``chn`` | ``setposition``<br>``setpos``<br>``position``<br>``pos`` | Manage Channels | ``[int] Position``<br><br>``(optional) [channel] Channel/Category (def: current)``<br><br>``(optional) [text] Reason`` | Change the position of the given channel in the guild channel list. | ``!channel setposition 2 #somechannel`` |
| ``channel``<br>``channels``<br>``c``<br>``chn`` | ``settopic``<br>``t``<br>``sett``<br>``topic`` | Manage Channels | ``[string] Topic``<br><br>``(optional) [channel] Channel/Category (def: current)``<br><br>``(optional) [text] Reason`` | Set a new channel topic. If channel is not given as argument, modifies the current channel. | ``!channel settopic Welcome to my channel!``<br><br>``!channel settopic "My topic" "Some Channel Name"`` |
|   |   |   |   |   |   |   |
| ``guild``<br>``server``<br>``g`` | ``info``<br>``i``<br>``information`` |  |  | Get guild information. | ``!guild info`` |
| ``guild``<br>``server``<br>``g`` | ``listmembers``<br>``memberlist``<br>``lm``<br>``members`` | Manage Guild |  | View guild member list. | ``!guild memberlist`` |
| ``guild``<br>``server``<br>``g`` | ``log``<br>``auditlog``<br>``viewlog``<br>``getlog``<br>``getlogs``<br>``logs`` | View Audit Log |  | Get guild audit logs. | ``!guild log`` |
...
| ``guild``<br>``server``<br>``g`` | ``prune``<br>``p``<br>``clean`` | Administrator (user)<br><br>Kick Members (bot) | ``(optional) [int] Days inactive (def: 7)``<br><br>``(optional) [text] Reason`` | Kick members who weren't active in given amount of days (1-7). | ``!guild prune``<br><br>``!guild prune 5`` |
...
|   |   |   |   |   |   |   |
| ``message``<br><br>``messages``<br>``m``<br>``msg``<br>``msgs`` | ``attachments``<br>``files``<br>``la``<br>``a`` |  | ``(optional) [id] Message ID (def: last message ID)`` | View message attachments. | ``!message attachments`` |
| ``message``<br><br>``messages``<br>``m``<br>``msg``<br>``msgs`` | ``delete``<br>``-``<br>``d``<br>``del``<br>``prune`` | Administrator (user)<br><br>Manage messages (bot) | ``[int] Amount (def: 5)``<br><br>``(optional) [text] Reason`` | Delete ``Amount`` messages from the current channel. | ``!messages delete 100 Spam`` |
| ``message``<br><br>``messages``<br>``m``<br>``msg``<br>``msgs`` | ``deletefrom``<br>``-user``<br>``-u``<br>``du``<br>``deluser``<br>``dfu`` | Administrator (user)<br><br>Manage messages (bot) | ``[user] User``<br><br>``[int] Amount (def: 5)``<br><br>``(optional) [text] Reason`` | Delete ``Amount`` messages from ``User`` in the current channel. | ``!messages deletefrom @Someone 100`` |
| ``message``<br><br>``messages``<br>``m``<br>``msg``<br>``msgs`` | ``deletereactions``<br>``-reactions``<br>``-r``<br>``delreactions``<br>``dr`` | Administrator (user)<br><br>Manage messages (bot) | ``(optional) [id] Message ID (def: last message ID)``<br><br>``(optional) [text] Reason`` | Delete all reactions from the given message. | ``!messages deletereactions 406201063792967680`` |
| ``message``<br><br>``messages``<br>``m``<br>``msg``<br>``msgs`` | ``deleteregex``<br>``-regex``<br>``-r``<br>``delregex``<br>``drx`` | Administrator (user)<br><br>Manage messages (bot) | ``[string] Regex``<br><br>``[int] Amount (def: 5)``<br><br>``(optional) [text] Reason`` | Delete ``Amount`` messages that match pattern ``Regex`` in the current channel (case ignored) | ``!messages deleteregex s+p+a+m+ 100`` |
| ``message``<br><br>``messages``<br>``m``<br>``msg``<br>``msgs`` | ``listpinned``<br>``lp``<br>``listpins``<br>``listpin``<br>``pinned`` |  |  | List pinned messages in the current channel. | ``!messages listpinned `` |
| ``message``<br><br>``messages``<br>``m``<br>``msg``<br>``msgs`` | ``modify``<br>``edit``<br>``mod``<br>``e``<br>``m`` | Manage Messages  | ``[id] Message ID``<br><br>``[text] New content`` | Edit the content of the message to a given content. | ``!messages edit 406201063792967680 Some new text`` |
| ``message``<br><br>``messages``<br>``m``<br>``msg``<br>``msgs`` | ``pin``<br>``p`` | Manage Messages  | ``(optional) [id] Message ID (def: last message ID)`` | Pin message with a given ID. If the message ID is not provided, the last message before the command invocation will be pinned. | ``!messages pin``<br><br>``!messages pin 406201063792967680`` |
| ``message``<br><br>``messages``<br>``m``<br>``msg``<br>``msgs`` | ``unpin``<br>``up`` | Manage Messages  | ``[int] Index (starting from 1)`` | Unpin pinned message with index ``Index`` in pinned message list (1 being the last pinned message). | ``!messages unpin 3`` |
| ``message``<br><br>``messages``<br>``m``<br>``msg``<br>``msgs`` | ``unpinall``<br>``upa`` | Manage Messages  |  | Unpin all pinned messages. | ``!messages unpinall`` |
|   |   |   |   |   |   |

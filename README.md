...
| ``channel``<br>``channels``<br>``c``<br>``chn`` | ``modify``<br>``edit``<br>``mod``<br>``m``<br>``e`` | Manage Channels | ``(optional) [int] User limit``<br><br>``(optional) [int] Bitrate (voice only)``<br><br>``(optional) [text] Reason`` | Modify current channel. Set 0 if you wish to keep the values as they are. | ``!channel modify 5 64000 "reason"``<br><br>``!channel mod 0 32000`` |

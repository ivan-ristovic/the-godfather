...
| ``channel``<br>``channels``<br>``c``<br>``chn`` | ``createtext``<br>``createt``<br>``+``<br>``+t``<br>``maket``<br>``newt``<br>``addt`` | Manage Channels | ``[string] Name``<br><br>``(optional) [channel] Parent Channel (def: none)`` | Create new text channel. *Note: Discord does not allow spaces in text channel name.* | ``!channel createtext spam`` |

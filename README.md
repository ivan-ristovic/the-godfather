...
| ``message``<br><br>``messages``<br>``m``<br>``msg``<br>``msgs`` | ``deletefrom``<br>``-user``<br>``du``<br>``deluser``<br>``dfu`` | Administrator (user)<br><br>Manage messages (bot) | ``[user] User``<br><br>``[int] Amount (def: 5)``<br><br>``(optional) [text] Reason`` | Delete ``Amount`` messages from ``User`` in the current channel. | ``!messages deletefrom @Someone 100`` |
| ``message``<br><br>``messages``<br>``m``<br>``msg``<br>``msgs`` | ``deleteregex``<br>``-regex``<br>``dr``<br>``delregex``<br>``dfr`` | Administrator (user)<br><br>Manage messages (bot) | ``[string] Regex``<br><br>``[int] Amount (def: 5)``<br><br>``(optional) [text] Reason`` | Delete ``Amount`` messages that match pattern ``Regex`` in the current channel (case ignored) | ``!messages deleteregex s+p+a+m+ 100`` |

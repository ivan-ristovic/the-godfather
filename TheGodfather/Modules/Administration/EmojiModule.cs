...
public sealed class EmojiModule : TheGodfatherModule
{
        ...
        [Description(TranslationKey.desc_emoji_info)] DiscordEmoji emoji)
    {
        try {
            ...
            emb.AddLocalizedField(TranslationKey.str_created_by, gemoji.User?.ToDiscriminatorString(), true);
            emb.AddLocalizedField(TranslationKey.str_animated, gemoji.IsAnimated, true);
            emb.AddLocalizedField(TranslationKey.str_managed, gemoji.IsManaged, true);
        ...
        [RemainingText][Description(TranslationKey.desc_rsn)] string? reason = null)
    {
        ...
        try {
            ...
            await ctx.Guild.ModifyEmojiAsync(gemoji, newname, reason: ctx.BuildInvocationDetailsString(reason));

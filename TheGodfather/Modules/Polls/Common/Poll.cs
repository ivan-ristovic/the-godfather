...
public class Poll : IChannelEvent
{
    ...
    public virtual async Task RunAsync(LocalizationService lcs)
    {
        ...
        this.IsRunning = true;
        DiscordMessage msgHandle = await this.Channel.SendMessageAsync(this.ToDiscordEmbed(lcs));
        ...
        while (!this.cts.IsCancellationRequested) {
            try {
                ...
                if (msgs.Any() && msgs.Single().Id != msgHandle.Id) {
                    ...
                    msgHandle = await this.Channel.SendMessageAsync(this.ToDiscordEmbed(lcs));
                } else {
                    await msgHandle.ModifyAsync(this.ToDiscordEmbed(lcs));
                }
            } catch {
                msgHandle = await this.Channel.SendMessageAsync(this.ToDiscordEmbed(lcs));
            }

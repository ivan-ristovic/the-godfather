...
public sealed class ReactionsService : ITheGodfatherService
{
    ...
    {
        return this.ereactions.TryGetValue(gid, out ConcurrentHashSet<EmojiReaction>? ers)
            ? ers.ToList()
            : Array.Empty<EmojiReaction>();
    }
        ...
        await using (TheGodfatherDbContext db = this.dbb.CreateContext()) {
            ...
            try {
                ...
                foreach (string trigger in triggers.Select(t => t.ToLowerInvariant())) {
                    ...
                    if (!er.AddTrigger(trigger, regex))
    ...
    {
        return this.treactions.TryGetValue(gid, out ConcurrentHashSet<TextReaction>? trs)
            ? trs.ToList()
            : Array.Empty<TextReaction>();
    }
    ...
    public async Task<bool> AddTextReactionAsync(ulong gid, string trigger, string response, bool regex)
    {
        ...
        await using (TheGodfatherDbContext db = this.dbb.CreateContext()) {
            ...
            try {
                ...
                if (!tr.AddTrigger(trigger, regex))

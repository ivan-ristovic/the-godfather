...
{
    private static readonly Regex _cbRegex = new(@"```(cs\n)?(?<code>[^`]+)```", RegexOptions.Compiled | RegexOptions.IgnoreCase);
    private static readonly ImmutableArray<string> _usings = new[] {
    ...
    public static Type? CompileCommand(string code)
    {
        ...
        Type? moduleType = outerType?.GetNestedTypes().FirstOrDefault(x => x.BaseType == typeof(TheGodfatherModule));

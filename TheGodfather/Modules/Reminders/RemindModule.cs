    ...
    public partial class RemindModule : TheGodfatherServiceModule<SchedulingService>
    {
        ...
        private Task PaginateRemindersAsync(CommandContext ctx, IEnumerable<Reminder> reminders, DiscordChannel? chn = null)
        {
            ...
            return ctx.PaginateAsync(reminders, (emb, r) => {
                ...
                emb.AddLocalizedField(TranslationKey.str_id, r.Id, inline: true);
                if (r.IsRepeating)
                    emb.AddLocalizedField(TranslationKey.str_repeating, r.RepeatInterval.Humanize(culture: culture), inline: true);

                if (r.TimeUntilExecution < TimeSpan.FromDays(1))
                    emb.AddLocalizedField(TranslationKey.str_executes_in, r.TimeUntilExecution.Humanize(3, culture: culture, minUnit: TimeUnit.Second), inline: true);
                else
                    emb.AddLocalizedTimestampField(TranslationKey.str_exec_time, r.ExecutionTime, inline: true);

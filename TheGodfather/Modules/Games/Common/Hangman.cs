...
namespace TheGodfather.Modules.Games
{
    public class Hangman : ChannelEvent
    {
        ...
        {
            _msg = await Channel.EmbedAsync("Game starts!", StaticDiscordEmoji.Joystick)
            ...
            if (IsTimeoutReached) {
                Winner = null;
                await Channel.EmbedAsync($"Nobody replies so I am stopping the game... The word was: {Formatter.Bold(_word)}", StaticDiscordEmoji.Joystick)
                    .ConfigureAwait(false);
                return;
            }

            if (_lives > 0) {
                await Channel.EmbedAsync($"{Winner.Mention} won the game!", StaticDiscordEmoji.Joystick)
                    .ConfigureAwait(false);
            } else {
                Winner = _initiator;
                await Channel.EmbedAsync($"Nobody guessed the word so {Winner.Mention} won the game! The word was: {Formatter.Bold(_word)}", StaticDiscordEmoji.Joystick)
                    .ConfigureAwait(false);
            }
        }

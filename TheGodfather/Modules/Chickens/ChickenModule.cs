...
public partial class ChickenModule : TheGodfatherServiceModule<ChickenService>
{
        ...
        [Description(TranslationKey.desc_member)] DiscordMember? member = null)
    {
        ...
        await ctx.RespondWithLocalizedEmbedAsync(emb => {
            ...
            emb.AddLocalizedField(TranslationKey.str_owner, chicken.Owner?.Mention ?? chicken.UserId.ToString(), true);
            emb.AddLocalizedField(TranslationKey.str_value, $"{chicken.SellPrice:n0}", true);
            emb.AddLocalizedField(TranslationKey.str_stats, chicken.Stats.ToString(), true);
            if (chicken.Stats.Upgrades?.Any() ?? false)
                emb.AddLocalizedField(TranslationKey.str_upgrades, chicken.Stats.Upgrades.Select(u => u.Upgrade.Name).JoinWith(", "), true);
    ...
    private async Task PrintChickensAsync(CommandContext ctx, IEnumerable<Chicken> chickens)
    {
        ...
        await ctx.PaginateAsync(chickens, (emb, c) => {
            ...
            emb.AddLocalizedField(TranslationKey.str_chicken_str, $"{c.BareStrength} ({c.Stats.TotalStrength})", true);
            emb.AddLocalizedField(TranslationKey.str_chicken_vit, $"{c.Vitality}/{c.BareMaxVitality} ({c.Stats.TotalMaxVitality})", true);

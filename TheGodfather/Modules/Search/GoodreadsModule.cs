...
public sealed class GoodreadsModule : TheGodfatherServiceModule<GoodreadsService>
{
        ...
        [RemainingText][Description(TranslationKey.desc_query)] string query)
    {
        ...
        await ctx.PaginateAsync(res.Results, (emb, r) => {
            ...
            emb.AddLocalizedField(TranslationKey.str_author, r.Book.Author.Name, true);
            emb.AddLocalizedField(TranslationKey.str_rating, r.AverageRating, true);
            emb.AddLocalizedField(TranslationKey.str_books_count, r.BooksCount, true);
            if (DateTimeOffset.TryParse($"{r.PublicationDayString}.{r.PublicationMonthString}.{r.PublicationYearString}", out DateTimeOffset dt))
                emb.AddLocalizedField(TranslationKey.str_published, dt.Humanize(culture: this.Localization.GetGuildCulture(ctx.Guild.Id)), true);
            emb.AddLocalizedField(TranslationKey.str_work_id, r.Id, true);
            emb.AddLocalizedField(TranslationKey.str_book_id, r.Book.Id, true);
            emb.AddLocalizedField(TranslationKey.str_reviews, r.TextReviewsCount, true);

...
public sealed class RssFeedsService : DbAbstractionServiceBase<RssFeed, int>
{
    ...
    public override RssFeed EntityFactory(int id) => new() { Id = id };
    ...
    public async Task<RssFeed?> GetByUrlAsync(string url)
    {
        ...
        RssFeed? feed = await db.RssFeeds.AsQueryable().SingleOrDefaultAsync(f => f.Url == url);
        return feed;
    }
    ...
    public sealed class RssSubscriptionService : DbAbstractionServiceBase<RssSubscription, (ulong gid, ulong cid), int>
    {
        ...
        public override RssSubscription EntityFactory((ulong gid, ulong cid) grid, int id) => new() { ChannelId = grid.cid, GuildId = grid.gid, Id = id };
        ...
        public async Task<RssSubscription?> GetByNameAsync((ulong gid, ulong cid) grid, string name)
        {
            ...
            await using TheGodfatherDbContext db = this.dbb.CreateContext();
            RssSubscription? sub = await this.GroupSelector(db.RssSubscriptions, grid)
                .Include(s => s.Feed)

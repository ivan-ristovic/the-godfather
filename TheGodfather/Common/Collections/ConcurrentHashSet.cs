...
public class ConcurrentHashSet<T> : IReadOnlyCollection<T>, ICollection<T>
{
    ...
    public int Count {
        get {
            ...
            try {
                ...
                for (int i = 0; i < this._tables.CountPerLock.Length; i++) count += this._tables.CountPerLock[i];
            } finally {
        ...
        get {
            ...
            try {
                ...
                for (int i = 0; i < this._tables.CountPerLock.Length; i++)
                    if (this._tables.CountPerLock[i] != 0) return false;
            } finally {
                this.ReleaseLocks(0, acquiredLocks);
            }
    ...
    public bool Contains(T item)
    {
        ...
        while (current != null) {
            if (hashcode == current.Hashcode && this._comparer.Equals(current.Item, item)) return true;
    ...
    public bool TryRemove(T item)
    {
        ...
        while (true) {
            ...
            lock (tables.Locks[lockNo]) {
                ...
                if (tables != this._tables) continue;
    ...
    void ICollection<T>.CopyTo(T[] array, int arrayIndex)
    {
        ...
        try {
            ...
            for (int i = 0; i < this._tables.Locks.Length && count >= 0; i++) count += this._tables.CountPerLock[i];
    ...
    {
        foreach (T item in collection) this.AddInternal(item, this._comparer.GetHashCode(item), false);

        if (this._budget == 0) this._budget = this._tables.Buckets.Length / this._tables.Locks.Length;
    }
    ...
    private bool AddInternal(T item, int hashcode, bool acquireLock)
    {
        while (true) {
            ...
            try {
                ...
                for (Node current = tables.Buckets[bucketNo]; current != null; current = current.Next) {
                    Debug.Assert((previous == null && current == tables.Buckets[bucketNo]) || previous.Next == current);
                    if (hashcode == current.Hashcode && this._comparer.Equals(current.Item, item)) return false;
                    previous = current;
                }
                ...
                //
                if (tables.CountPerLock[lockNo] > this._budget) resizeDesired = true;
            } finally {
                if (lockTaken)
            ...
            //
            if (resizeDesired) this.GrowTable(tables);
    ...
    private void GrowTable(Tables tables)
    {
        ...
        try {
            ...
            if (tables != this._tables) // We assume that since the table reference is different, it was already resized (or the budget
            ...
            long approxCount = 0;
            for (int i = 0; i < tables.CountPerLock.Length; i++) approxCount += tables.CountPerLock[i];
            ...
            if (approxCount < tables.Buckets.Length / 4) {
                ...
                if (this._budget < 0) this._budget = int.MaxValue;
                return;
            }
    ...
    private void ReleaseLocks(int fromInclusive, int toExclusive)
    {
        ...
        for (int i = fromInclusive; i < toExclusive; i++) Monitor.Exit(this._tables.Locks[i]);
    }

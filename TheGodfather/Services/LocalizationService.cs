...
{
    private static readonly HashSet<string> _tzIds = new(TimeZoneInfo.GetSystemTimeZones().Select(tz => tz.Id));
    ...
    public string GetCommandDescription(ulong? gid, string command)
    {
        ...
        if (this.cmddesc is null || !this.cmddesc.TryGetValue(locale, out ImmutableDictionary<string, string>? desc))
            throw new LocalizationException($"Failed to find locale {locale}");

        if (!desc.TryGetValue(command, out string? localizedDesc))

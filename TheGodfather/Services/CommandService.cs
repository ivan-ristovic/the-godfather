...
public sealed class CommandService : ITheGodfatherService
{
    ...
    public void LoadData(string root)
    {
        ...
        var mods = new Dictionary<ModuleType, ConcurrentHashSet<string>>();
        ...
        this.commands = new ConcurrentDictionary<string, CommandInfo>(cmds);
        this.modules = mods.ToImmutableDictionary();
        ...
        ModuleType ParseModuleType(FileInfo fi)
        {
            ...
            if (!mods.ContainsKey(module))
                mods.Add(module, new ConcurrentHashSet<string>());
            return module;
        }
        ...
        void ReadCommands(FileInfo fi, ModuleType module)
        {
            try {
                ...
                Dictionary<string, CommandInfo> parsedCmds = JsonConvert.DeserializeObject<Dictionary<string, CommandInfo>>(json) 
                                                             ?? throw new JsonSerializationException();

                foreach ((string cmd, CommandInfo info) in parsedCmds) {
                    info.Module = module;
    ...
    {
        [JsonProperty("usage")]
        public List<List<string>> UsageExamples { get; set; } = new();
